...
$block_defaults = [
	'id'       => ! empty( $block['anchor'] ) ? $block['anchor'] : 'cards-with-image-mask-' . $block['id'],
	'class'    => [ 'acf-block', 'cards-with-image-mask', 'position-relative', 'overflow-hidden', $background_defaults['class'] ],
	'settings' => [
...
$column_class    = join( ' ', [
	'cards-with-image-mask-inner',
	$block_settings['settings']['heading_color'],
	$block_settings['settings']['text_color'],
	$content_width_class
] );
if(!empty($block_content['section_header']) || !empty($block_content['cards']) || !empty($block_content['buttons'])) : ?>
					...
					<div class="cards"><?php
						...
						if ( ! empty( $block_content['cards'] ) ) :
							...
							foreach ( $block_content['cards'] as $cards ):
								...
								print_module( 'card-with-image-mask', $cards );
								echo '</div>';
