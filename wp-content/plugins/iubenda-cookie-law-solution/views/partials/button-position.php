				...
				<fieldset class="paste_embed_code tabs tabs--style2">
					<ul class="tabs__nav">
						...
						foreach ( iubenda()->languages as $k => $v ) :
							...
							?>
							<li class="tabs__nav__item <?php echo esc_html( $_status ); ?>" data-target="tab-<?php echo esc_html( $k ); ?>" data-group="language-tabs">
					...
					foreach ( $languages as $lang_id => $v ) :
						...
						?>
						<div data-target="tab-<?php echo esc_html( $lang_id ); ?>" class="tabs__target <?php echo esc_html( $_status ); ?>" data-group="language-tabs">
							<textarea readonly class='form-control text-sm m-0 iub-pp-code' rows='4'><?php echo esc_html( $code ); ?></textarea>
						</div>
					<?php endforeach; ?>
