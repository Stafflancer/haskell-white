...
class Radar_Service {
	...
	private function send_radar_progress_request() {
		...
		if ( intval( iub_array_get( $iubenda_radar_api_configuration, 'next_trial' ) ) > time() ) {
			if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
				$next_request_in_sec = intval( iub_array_get( $iubenda_radar_api_configuration, 'next_trial' ) ) - ( time() );
