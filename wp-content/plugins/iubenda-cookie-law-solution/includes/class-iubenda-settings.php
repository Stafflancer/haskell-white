...
class Iubenda_Settings {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_save_public_api_key', array( new Quick_Generator_Service(), 'save_public_api_key' ) );
		add_action( 'wp_ajax_auto_detect_forms', array( new Quick_Generator_Service(), 'auto_detect_forms' ) );
		add_action( 'wp_ajax_ajax_save_options', array( new Quick_Generator_Service(), 'ajax_save_options' ) );
	...
	public function admin_enqueue_scripts( $page ) {
		...
		$iub_js_vars = array(
			...
			'iub_public_api_key_nonce'           => wp_create_nonce( 'iub_public_api_key_nonce' ),
	 ...
	 */
	public function update_options( $data = array() ) {
		iub_verify_ajax_request( 'iub_update_options', 'iub_update_options_nonce' );
		...
		$iubenda_cookie_law_solution_status = (string) iub_array_get( $result, 'iubenda_activated_products.iubenda_cookie_law_solution', 'false' );
		if ( 'true' !== $iubenda_cookie_law_solution_status ) {
			echo wp_json_encode(
				array(
					'status'       => 'error',
		...
		foreach ( (array) iub_array_get( $result, 'iubenda_activated_products', array() ) as $product_name => $product_status ) {
			if ( 'false' === (string) $product_status || 'iubenda_consent_solution' === (string) $product_name ) {
				continue;
			}

			// Count valid codes per $product_name and return error if doesn't have at least 1 valid code.
			if ( count( array_filter( (array) iub_array_get( $result, "codes_statues.{$product_name}_codes", array() ) ) ) === 0 ) {
				echo wp_json_encode(
					array(
						'status'       => 'error',
						'responseText' => "( {$product_name} ) At least one code must be valid.",
					)
				);
				wp_die();
			}
	 ...
	 * @return bool
	 */
	public function check_embed_code( $code_name, $code, string $section ) {
		if ( substr( $code_name, 0, 5 ) === 'code_' && empty( $code ) ) {
			return false;
		}

		if ( 'iubenda_privacy_policy_solution' === $section || 'iubenda_terms_conditions_solution' === $section ) {
			if ( ! iubenda()->parse_tc_pp_configuration( stripslashes_deep( $code ) ) ) {
				return false;
			}
		}

		if ( 'iubenda_cookie_law_solution' === $section && substr( $code_name, 0, 5 ) === 'code_' ) {
			return ! empty( $code );
		}
		...
		foreach ( $products as $product_name => $product_key ) {
			...
			if ( iub_array_get( $data, "{$product_name}_status" ) && 'true' === (string) iub_array_get( $data, "{$product_name}_status" ) ) {
				...
				if ( 'cons' === $product_key ) {
					...
					$public_api_key = sanitize_text_field( iub_array_get( $data, 'public_api_key' ) ?? null );
					...
					foreach ( $languages as $lang_id => $lang_name ) {
						...
						if ( 'cs' === $product_key && ! empty( $parsed_code ) ) {
							...
							if ( iub_array_get( $parsed_code, 'siteId' ) ?? null ) {
								$result['site_id'] = sanitize_key( iub_array_get( $parsed_code, 'siteId' ) );
							}

							// getting public id to save it into Iubenda global option by lang.
							if ( iub_array_get( $parsed_code, 'cookiePolicyId' ) ?? null ) {
								$result['public_ids'][ $lang_id ] = sanitize_key( iub_array_get( $parsed_code, 'cookiePolicyId' ) );
							}
						}
						...
						if ( in_array( $product_key, array( 'pp', 'tc' ), true ) ) {
							...
							if ( $parsed_code ) {
								$result['public_ids'][ $lang_id ] = sanitize_key( iub_array_get( $parsed_code, 'cookie_policy_id' ) );
								$product_option['button_style']   = sanitize_text_field( iub_array_get( $parsed_code, 'button_style' ) );
							}
	...
	public function save_init_prepared_product_options( $products, $result ) {
		...
		foreach ( $products as $product_name => $product_key ) {
			...
			update_option( $product_name, $new_product_option );
		...
		update_option( 'iubenda_activated_products', $new_iubenda_activated_products );
